...
unsigned long *stats; //该数组里面记录了出系统和进系统的时间差
unsigned statscount,  //计算时间的时候的总包数
statssz;
         ...
         uint16_t lport, uint16_t rport)
{
    ...
    if (hash_get_rem(sessions, laddr.s_addr, raddr.s_addr, lport, rport, &start))
    {
        newstat = (tv.tv_sec - start.tv_sec) * 1000000 +
                    (tv.tv_usec - start.tv_usec); //这就是出系统的时间-进系统的时间戳，也就是进入本系统和出本系统的时间差
...
/*** Results ***/
struct stats_results { //一段时间内的统计信息存入到该结构，见get_flush_stats  outbound
    unsigned long *stats;
...
get_flush_stats(void) {  //时延统计关键函数见见get_flush_stats  outbound
    ...
    ret->stats = stats;
    ret->statscount = statscount; //由ret->statscount指向统计好的各种信息  这段时间段内已经统计好的时延放在了该数组里，在该函数wait进行统计
    ret->statssz = statssz;
    
    ret->sorted = 0;
    
    stats = malloc((statssz = INITIAL_STAT_SZ) * sizeof(unsigned long)); //开辟新的数组用来统计下一次时间周期的时间差值
    if (!stats)
        abort();
    statscount = 0;
